...
function getToolLabel(toolName: string, args: Record<string, string>, isDone: boolean): string {
  ...
  if (toolName === "str_replace_editor") {
    ...
    if (cmd === "create") return isDone ? `Archivo creado: ${filename}` : `Creando ${filename}…`;
    if (cmd === "str_replace" || cmd === "insert") return isDone ? `Archivo editado: ${filename}` : `Editando ${filename}…`;
    if (cmd === "view") return isDone ? `Archivo leído: ${filename}` : `Leyendo ${filename}…`;
  }

  if (toolName === "file_manager") {
    const cmd = args?.command;
    if (cmd === "rename") return isDone ? "Archivo renombrado" : "Renombrando archivo…";
    if (cmd === "delete") return isDone ? `Archivo eliminado: ${filename}` : `Eliminando ${filename}…`;
  }

  return isDone ? "Listo" : "Procesando…";
}
                          ...
                          case "tool-invocation":
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {isDone ? (
                                  <>
                                    <FileCode2 className="w-3 h-3 text-emerald-500 flex-shrink-0" />
                                    <span className="text-neutral-600">{getToolLabel(tool.toolName, tool.args as Record<string, string>, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600 flex-shrink-0" />
                                    <span className="text-neutral-600">{getToolLabel(tool.toolName, tool.args as Record<string, string>, false)}</span>
                                  </>
                                )}
                              </div>
                            );
